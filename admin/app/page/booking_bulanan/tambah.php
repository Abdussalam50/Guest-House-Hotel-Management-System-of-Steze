...
<?php
$listHotel = mysql_query("SELECT * FROM data_hotel ORDER BY nama ASC");
if (isset($_COOKIE['id_hotel'])) {
    $idHotel = decrypt($_COOKIE["id_hotel"]);
} elseif (isset($_GET['id_hotel']) && $_GET['id_hotel'] != "") {
...
?>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            let htmlList = `
        <a href="../home/" class="btn-close-modal">Tutup</a>
    ...
    `;
            ...
            <?php while ($h = mysql_fetch_assoc($listHotel)) { ?>
                htmlList += `
        <div style="
            padding:15px;
            border-radius:12px;
        ...
        "
        onmouseover="this.style.background='#f2f8ff'; this.style.borderColor='#7abaff';"
        onmouseout="this.style.background='white'; this.style.borderColor='#dcdcdc';"
        onclick="pilihHotel('<?php echo $h['id_hotel']; ?>')"
        >
                <br>
            <div style="font-size:24px;color:#3498db;margin-bottom:8px">
                <i class="fa-solid fa-hotel" style="font-size: 30px;"></i>
            </div>

            <div style="font-size:15px;font-weight:bold;color:#333;min-height:10px">
                <?php echo addslashes($h['nama']); ?>
            </div>

            <div style="font-size:12px;color:#666;margin-top:6px;min-height:50px">
                <?php echo addslashes($h['alamat']); ?>
            </div>
    ...
    `;
        ...
        function pilihHotel(id) {
            window.location.href = "?input=tambah&id_hotel=" + id;
        }
    </script>
                        ...
                        <div class="mb-4">
                            <div class="cardcheckin-body">
                                <div class="d-flex justify-content-between align-items-center mb-4">
                                    ...
                                    <div class="d-flex align-items-center gap-3" style="cursor:pointer" onclick="window.location.href='index.php?input=tambah'">
                                            ...
                                            <div class="cardcheckin-body">
                                                ...
                                                <script>
                                                    ...
                                                    function loadKamar() {
                                                        const tanggalCheckin = document.getElementById('waktu_checkin').value;
                                                        const tanggalCheckout = document.getElementById('waktu_check_out').value;
                                                        const idHotel = "<?php echo $idHotel ?>";
                            ...
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <input type="hidden" onclick='pilih_metode_deposit()' class="btn btn-secondary">
                                        ...
                                        <input type="hidden" class="form-check-input me-2" id='totalplusdepo'>
                                        ...
                                        <div class="mb-2" id="prediksi_buttons" style="padding: 4px 12px;font-size: 0.75rem;opacity: 0.8;visibility: hidden;border-color: rgb(204, 204, 204);"></div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const elements = {
            ...
            days: document.getElementById('jumlah_bulan'),
        ...
        // GANTI SELURUH FUNGSI updateCheckoutDate() dengan ini:
        const updateCheckoutDate = () => {
            // const checkinDate = new Date(elements.checkin.value);
            const jumlahBulan = parseInt(elements.days.value) || 1;

            // if (!checkinDate) return;

            // // Hitung tanggal checkout dengan menambah bulan (bukan hari!)
            // const checkoutDate = new Date(checkinDate);
            // checkoutDate.setMonth(checkinDate.getMonth() + jumlahBulan);

            // // Format ke YYYY-MM-DD
            // elements.checkout.value = checkoutDate.toISOString().split('T')[0];

            // // Update total harga (jika masih pakai cara lama, ganti juga nanti)
            // // refreshListKamar(); // pastikan semua kamar ikut update jumlah bulannya
            updateJumlahbulanTransaksi(idTransaksi, jumlahBulan); // ini sudah benar
        };


        const updateJumlahbulanTransaksi = (idTransaksi, jumlahbulan) => {
            const formData = new FormData();
            formData.append('id_transaksi', idTransaksi);
            formData.append('jumlah_bulan', jumlahbulan);

            fetch("update_jumlah_bulan.php", {
                    method: "POST",
                    body: formData
                })
                .then(res => res.json())
                .then(data => {
                    if (data.status === "success") {
                        ...
                        console.error("Gagal update jumlah bulan:", data.msg);
                    }
                })
        ...
        const inputbulan = document.getElementById('jumlah_bulan');
