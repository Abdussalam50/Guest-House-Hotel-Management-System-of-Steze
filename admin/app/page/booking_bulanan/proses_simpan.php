...
$jumlah_bulan = is_numeric($_POST['jumlah_bulan']) ? $_POST['jumlah_bulan'] : 1;
...
if ($jml == 0) {
  ...
  $harga_kamar_total = $harga_kamar_bulanan * $jumlah_bulan;
  ...
  while ($row = mysql_fetch_assoc($query)) {
    ...
    $harga_kamar_total = $harga_kamar_total + ((int)$row['harga_kamar_bulanan'] * $jumlah_bulan);
...
$query_transaksi = mysql_query("INSERT INTO data_booking VALUES (
...
'$jam_checkin','$jam_checkout','$jumlah_bulan','$jenis_transaksi','$id_admin','$nama_admin','$id_hotel','$nama_hotel',
...
$sql_transaksi = "INSERT INTO data_booking VALUES (
'$id_transaksi','$id_pelanggan','$id_kamar','$waktu_checkin','$waktu_check_out',
'$no_rekening','$harga_kamar_total','$id_metode_pembayaran','$metode_pembayaran','$jumlah_dewasa','$jumlah_anak_anak',
'$discount','$status_transaksi','$nama_pelanggan','$no_hp_pelanggan','$no_kamar','$tipe_kamar',
'$jam_checkin','$jam_checkout','$jumlah_bulan','$jenis_transaksi','$id_admin','$nama_admin','$id_hotel','$nama_hotel',
...
// INSERT DATA PEMASUKAN (BOOKING DP)
$id_pemasukan = id_otomatis("data_pemasukan", "id_pemasukan", "10");
$keterangan = 'Pembayaran Booking Bulanan A.n ' . $nama_pelanggan . ', Kamar Nomor ' . $no_kamar . ' (' . ($tipe_kamar) . ")";
...
if ($query_transaksi) {
  ...
  simpan_riwayat("data_booking", "id_transaksi", $id_transaksi, $sql_transaksi, $id_admin);
  ...
  ?>
    <script>
      window.location.href = "../checkout/notaA4-booking-bulanan.php?id_trx=<?php echo $id_transaksi ?>&status=checkin";
    </script>
  <?php
  } else {
  ?>
