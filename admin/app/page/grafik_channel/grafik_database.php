...
// Fetch channel transaction data
$sql = "
    SELECT dc.channel, COALESCE(COUNT(dt.id_transaksi), 0) AS total_transaksi
    FROM data_channel dc
    LEFT JOIN data_transaksi dt 
        ON dc.id_channel = dt.id_channel
        AND YEAR(dt.waktu_checkin) = '$selected_year'
        $filter_hotel
    GROUP BY dc.channel
    ORDER BY total_transaksi DESC
";
$result = mysql_query($sql) or die('Channel query failed: ' . mysql_error());
...
<head>
	<title>Grafik Jumlah Transaksi per Channel</title>
...
<body>
	<h3>
		Grafik Jumlah Transaksi per Channel (Tahun <?php echo htmlspecialchars($selected_year); ?>)
	...
	<!-- Tabel Data -->
	<h3>Data Transaksi per Channel</h3>
	<table>
		<tr>
			<th>Channel</th>
		...
		<?php for ($i = 0; $i < count($channel); $i++): ?>
			<tr>
				<td><?php echo htmlspecialchars($channel[$i]); ?></td>
