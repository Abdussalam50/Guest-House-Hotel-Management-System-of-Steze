...
if ($total_tambahan_non_pajak > 0) {
    ...
    // Update status kamar
    $q_list = mysql_query("SELECT id_kamar FROM data_transaksi_list_kamar WHERE id_transaksi='$id_transaksi'");
    while ($row = mysql_fetch_assoc($q_list)) {
        $id_k = mysql_real_escape_string($row['id_kamar']);
        mysql_query("UPDATE data_kamar SET status_kamar='Kosong' WHERE id_kamar='$id_k'");
    }
