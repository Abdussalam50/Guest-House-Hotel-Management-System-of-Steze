...
try {
    ...
    $datas = mysql_fetch_array(mysql_query("SELECT * FROM data_booking WHERE id_transaksi='$id_transaksi'"));

    $deposit = rupiah_format($datas['nominal_deposit'] == null ? 0 : $datas['nominal_deposit']);

    // Data hotel
    $id_hotel = baca_database("", "id_hotel", "SELECT * FROM data_booking WHERE id_transaksi='$id_transaksi'");
    ...
    $sisa_pembayaran   = ($datas['sisa_pembayaran'] == null ? 0 : $datas['sisa_pembayaran']);
    ...
    if (!$is_multi_kamar && !empty($id_kamar_field)) {
        ...
        $q = mysql_query("SELECT 
                ...
                tlk.harga_kamar_harian, tlk.total_harga_kamar, tlk.jumlah_hari
            FROM data_kamar dk
            JOIN data_tipe_kamar dtk ON dk.id_tipe_kamar = dtk.id_tipe_kamar
            LEFT JOIN data_booking_list_kamar tlk ON tlk.id_kamar = dk.id_kamar AND tlk.id_transaksi = '$id_transaksi'
        ...
        // MULTI KAMAR → ambil dari data_booking_list_kamar
        foreach ($decoded as $id_kamar_single) {
            ...
            $q = mysql_query("SELECT 
                    ...
                    tlk.jumlah_dewasa, tlk.jumlah_anak_anak,
                    tlk.harga_kamar_harian, tlk.total_harga_kamar, tlk.jumlah_hari
                FROM data_booking_list_kamar tlk
    ...
    <body id="form-rekap-pembayaran">
        ...
        <div class="row" style="display:flex;margin-right:0px;margin-left:0px;">
            ...
            <table style="border:collapse; width:50%;border:none;margin-bottom:10px;margin-left:auto;font-size:12px">
                ...
                <tr>
                    ...
                    <td>:</td>
                    <td>&nbsp;<font color="#2196f3"><b><?php echo $status_transaksi; ?></b></font>
                    </td>
                </tr>
                <tr>
        ...
        <div id='box' style='display:flex;width:100%;'>
            ...
            <table id="table-bottom" style="width:60%;float:right;border:none;font-size:12px">
                ...
                <tr>
                    <td style="text-align:left;padding:2px;font-weight:bold;color:#2196f3">DP (Down Payment)</td>
                    <td style='font-weight:700;color:#2196f3'>:</td>
                    <td style="text-align:right;padding:2px;color:#2196f3"><b>Rp <?php echo rupiah_format($nominal_bayar); ?></b></td>
                </tr>

                <tr>
                    <td style="text-align:left;padding:2px;font-weight:bold;">Sisa Pembayaran</td>
                    <td style='font-weight:700'>:</td>
                    <td style="text-align:right;padding:2px;"><b>Rp<?php echo rupiah_format($sisa_pembayaran); ?></b></td>
                </tr>
            </table>
        </div>
        ...
        if (!isset($_GET['id_trx'])) {
            ...
            mysql_query("UPDATE data_booking SET status_transaksi='Selesai' WHERE id_transaksi='$id_transaksi'");
        ...
        <?php if ($read != "detail") { ?>
            <div id="print-area-btn" class="no-print" style="margin-top:20px;text-align:right;">
                <button id="btnPrint" class="btn btn-primary" style="margin-right:10px;"><i class="fa fa-print"></i> Print</button>
