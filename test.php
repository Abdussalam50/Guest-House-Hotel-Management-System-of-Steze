...
<div class="container">
    ...
    <div class="mb-3">
        ...
        <input id="waktu_check_out" class="form-control " readonly>
    </div>
</div>
...
<script>
    ...
    // Ambil data schedule dari schedule.php
    fetch('schedule.php')
        .then(res => res.json())
        .then(data => {
            data.forEach(i => {
    ...
    function initFlatpickr() {
        fpCheckIn = flatpickr("#waktu_check_in", {
            ...
            onChange: function(_, dateStr) {
                ...
                if (parseInt(document.getElementById("jumlah_hari").value) > maxHari) {
                    Swal.fire({
                        title: "Jumlah hari tidak bisa dipilih",
                        html: `Dalam <b>${document.getElementById("jumlah_hari").value}</b> hari dari <b>${dateStr}</b> terdapat transaksi/booking sehingga jumlah hari tidak dapat digunakan.`,
                        icon: "warning"
                    }).then(() => resetInput());
                }
                hitungCheckout();
    ...
    function hitungCheckout() {
        ...
        checkout.setDate(checkin.getDate() + hari - 1);
        ...
        document.getElementById("waktu_check_out").value = toLocalDateString(checkout);
    }
